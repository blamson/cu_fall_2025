...
Removing that operator requires breaking up each coefficient into two parts so we let $b_j = b_j^+ - b_j^-$ where $b_j^+ , b_j^- \ge 0$. The absolute value then is just $|b_j| = b_j^+ + b_j^-$.
